...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('provider:getInstalled', async () => {
    ...
    if (settings.chatProvider.installed) {
      const providerId = settings.chatProvider.installed.id
      // 内置 provider：直接读取内置 manifest
      if (providerId === BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID) {
        const installed = await getBuiltinOpenaiCompatibleInstalledInfo()
        const dir = join(app.getAppPath(), 'resources', 'providers', BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID)
  ...
  ipcMain.handle('engine:updateConfig', async (_event, config) => {
    ...
    if (runtimeDevice) {
      // setApiKey/setBaseUrl/setModel 在 BoxSelectDevice 上是 no-op，对 RPADevice 才生效。
      runtimeDevice.setApiKey(settings.vision.apiKey)
      runtimeDevice.setBaseUrl(settings.vision.baseUrl)
      runtimeDevice.setModel(settings.vision.model)
  ...
  ipcMain.handle('engine:testConnection', async (_event, config) => {
    ...
    const baseUrl = config?.baseUrl || settings.vision.baseUrl || DEFAULT_VISION_BASE_URL
    const model = config?.model || settings.vision.model || DEFAULT_VISION_MODEL
...
async function startEngineCore(rawConfig?: any): Promise<SkillStartResult> {
  ...
  try {
    ...
    // 只有 doubao 共享视觉密钥，其他 provider 都有自己的 apiKey 配置
    const isDoubaoProvider = !settings.chatProvider.installed ||
      settings.chatProvider.installed.id === BUILTIN_DOUBAO_PROVIDER_ID
    const needsVisionKey = startupStrategy === 'vlm' || isDoubaoProvider
    ...
    // 根据 provider 类型加载
    let provider
    const installedProviderId = settings.chatProvider.installed?.id

    if (!installedProviderId) {
      // 默认走 doubao，共享视觉密钥
    ...
    } else if (installedProviderId === BUILTIN_DOUBAO_PROVIDER_ID) {
      // doubao 共享视觉密钥
      const loaded = await loadBuiltinDoubaoProvider({
        ...settings.chatProvider.config,
        apiKey: settings.vision.apiKey
      ...
      provider = loaded.provider
    } else if (installedProviderId === BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID) {
      // openai-compatible 需要用户自己配置 apiKey、baseUrl、model
      const config = settings.chatProvider.config
      if (!config?.apiKey) {
        return { ok: false, reason: 'missing_api_key', message: '请配置自定义 LLM 的接口密钥' }
      }
      if (!config?.baseUrl) {
        return { ok: false, reason: 'missing_base_url', message: '请配置自定义 LLM 的 API 地址' }
      }
      if (!config?.model) {
        return { ok: false, reason: 'missing_model', message: '请配置自定义 LLM 的模型名称' }
      }
      const loaded = await loadBuiltinOpenaiCompatibleProvider(config)
      provider = loaded.provider
    } else {
      // 外部安装的 provider
      if (!settings.chatProvider.installed) {
        return { ok: false, reason: 'no_provider', message: 'Provider 配置缺失' }
      }
      ...
      const loaded = await loadInstalledProvider(settings.chatProvider.installed, settings.chatProvider.config)
      provider = loaded.provider
    }
...
async function buildDevice(
  ...
  if (effective === 'vlm') {
    ...
    rpa.setApiKey(settings.vision.apiKey)
    rpa.setBaseUrl(settings.vision.baseUrl)
    rpa.setModel(settings.vision.model)
...
function normalizeSettings(raw: any): AppSettings {
  ...
  const oldModel = typeof raw?.model === 'string' && raw.model ? raw.model : DEFAULT_VISION_MODEL
  const oldBaseUrl = typeof raw?.baseUrl === 'string' && raw.baseUrl ? raw.baseUrl : DEFAULT_VISION_BASE_URL
  ...
  return {
    ...
    vision: {
      ...
      baseUrl: raw?.vision?.baseUrl || oldBaseUrl || '',
      model: raw?.vision?.model || oldModel || ''
    },
    chatProvider: {
