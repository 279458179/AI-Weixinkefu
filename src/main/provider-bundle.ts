...
export const BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID = 'openai-compatible'

const BUILTIN_PROVIDER_IDS = [BUILTIN_DOUBAO_PROVIDER_ID, BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID]
...
/** 内置 openai-compatible 的虚拟 installed 描述 */
export async function getBuiltinOpenaiCompatibleInstalledInfo(): Promise<InstalledProviderInfo | null> {
  const dir = getBuiltinProviderDir(BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID)
  const manifestFile = path.join(dir, 'manifest.json')
  try {
    const content = await readFile(manifestFile, 'utf8')
    const raw = validateManifest(JSON.parse(content))
    return {
      id: raw.id,
      name: raw.name,
      version: raw.version,
...
/** 通用的内置 provider 加载 */
export async function getBuiltinProviderInstalledInfo(providerId: string): Promise<InstalledProviderInfo | null> {
  if (providerId === BUILTIN_DOUBAO_PROVIDER_ID) {
    return getBuiltinDoubaoInstalledInfo()
  }
  if (providerId === BUILTIN_OPENAI_COMPATIBLE_PROVIDER_ID) {
    return getBuiltinOpenaiCompatibleInstalledInfo()
  }
  return null
}
...
/** 直接加载内置 openai-compatible provider */
export async function loadBuiltinOpenaiCompatibleProvider(
  providerConfig: Record<string, any>
): Promise<{ provider: ProviderAdapter; manifest: ProviderBundleManifest }> {
  const installed = await getBuiltinOpenaiCompatibleInstalledInfo()
  if (!installed) {
    throw new Error('内置 openai-compatible provider 资源缺失')
  }
  return loadInstalledProvider(installed, providerConfig)
}
