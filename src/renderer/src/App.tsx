...
const BUILTIN_PROVIDER_CATALOG: ProviderCatalogItem[] = [
  ...
  },
  {
    ...
    name: '自定义 LLM',
    description: '支持任意 OpenAI 兼容接口（OpenAI、DeepSeek、Claude、本地模型等）',
    version: '1.0.0',
    manifestUrl: 'builtin://openai-compatible',
    capabilities: ['chat'],
    configSchema: {
      fields: [
        {
          ...
          hint: 'OpenAI 兼容接口地址，如 https://api.openai.com/v1 或本地模型地址'
        },
        {
          key: 'apiKey',
          label: '接口密钥',
      ...
      <div className="card base-settings-card">
        ...
        <div className="form-group">
          ...
          <input
            ...
            autoComplete="off"
          />
          <div className="form-hint">视觉识别使用的模型名称</div>
        </div>
        ...
        <div className="form-group">
          ...
          <input
            ...
            autoComplete="off"
          />
          <div className="form-hint">API 端点地址</div>
        </div>
...
function AgentPanel(): React.JSX.Element {
  ...
  const persistProvider = useCallback(
    async (provider: ProviderCatalogItem, values: Record<string, string>) => {
      ...
      // openai-compatible 是内置 provider，直接保存配置
      if (provider.id === 'openai-compatible') {
        ...
        await window.electron?.invoke('engine:updateConfig', settings)
        setCurrentSettings(settings)
        setActiveId('openai-compatible')
        return true
      }

      // 其他外部 provider：通过 URL 安装
